...
make_packages_aur() {
    ...
    for _pkg2 in ${pkglist_aur[@]}; do
        echo  "cd ~ ; git clone https://aur.archlinux.org/${_pkg2}.git ; cd ${_pkg2} ; makepkg -cs " > ${work_dir}/${arch}/airootfs/aurbuild_temp/test.sh
        ${mkalteriso} ${mkalteriso_option} -w "${work_dir}/${arch}"  -D "${install_dir}" -r "chmod 777 /aurbuild_temp/test.sh" run
        ${mkalteriso} ${mkalteriso_option} -w "${work_dir}/${arch}"  -D "${install_dir}" -r "sudo -u aurbuild /aurbuild_temp/test.sh" run
        pkgf=`ls ${work_dir}/${arch}/airootfs/aurbuild_temp/${_pkg2}/*.pkg.tar.*`
