...
make_packages_repo() {
    local _pkg pkglist_args="-a ${arch} -k ${kernel} -c ${channel_dir} -l ${locale_name}"
...
make_packages_aur() {
    local _pkg pkglist_args="--aur -a ${arch} -k ${kernel} -c ${channel_dir} -l ${locale_name}"
    ...
    local _pkglist_aur=($("${tools_dir}/pkglist.sh" ${pkglist_args}))
