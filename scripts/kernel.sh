...
# Message functions
msg_error() {
    "${script_path}/scripts/msg.sh" -s 6 -a "kernel.sh" error "${1}"
}
...
gen_kernel_list() {
    ...
    local _list _kernel
    if [[ -n "${profile}" ]] && [[ -f "${profile}/kernel_list-${arch}" ]]; then
        _list="${profile}/kernel_list-${arch}"
    else
...
get() {
    ...
    if [[ -n "${profile}" ]] && [[ -f "${profile}/kernel-${arch}" ]]; then
        _kernel_config_file="${profile}/kernel-${arch}"
    else
...
ARGUMENT="${@}"
opt_short="a:p:hs"
opt_long="arch:,profile:,help,script"
