...
グローバル関数: Func_Global_[関数名]

[ Func_Global_Main_ask_questions内で使用する関数のみ ]
グローバル関数: Func_Global_Ask_[設定される変数名]
...
Func_Global_Main_wizard_language () {
...
Func_Global_Main_check_required_files () {
...
Func_Global_Main_load_default_config () {
    source "${Var_Global_Wizard_Env_script_path}/default.conf"
}

Func_Global_Main_check_yay(){
...
Func_Global_Main_install_dependent_packages () {
    #local pkg installed_pkg installed_ver check_pkg
    local Func_Local_checkpkg Var_Local_package
    ...
    for Var_Local_package in "${dependence[@]}"; do
        ...
        if Func_Local_checkpkg "${Var_Local_package}"; then
...
Func_Global_Main_guide_to_the_web () {
    if [[ "${Var_Global_Wizard_Option_language}"  = "jp" ]]; then
...
Func_Global_Main_run_keyring.sh () {
...
Func_Global_Main_remove_dependent_packages () {
    if [[ -n "${Var_Global_missing_packages[*]}" ]]; then
...
# Func_Global_Ask_japaneseの代わりにFunc_Global_Ask_localeを使用して下さい。
Func_Global_Ask_locale() {
    msg \
...
Func_Global_Ask_comp_option () {
    comp_option=""
    local Func_Local_comp_option
    case "${Var_Global_Build_comp_type}" in
        "gzip")
            Func_Local_comp_option() {
                local Var_Local_gzip_level Var_Local_gzip_window
                local Func_Local_gzip_level Func_Local_gzip_window

                Func_Local_gzip_level () {
        ...
        "lzo")
            Func_Local_comp_option () {
                local Func_Local_lzo_algorithm Var_Local_lzo_algorithm
                Func_Local_lzo_algorithm () {
                    msg \
                ...
                if [[ "${Var_Local_lzo_algorithm}" = "lzo1x_999" ]]; then                
                    local Func_Local_lzo_level Var_Local_lzo_level
                    Func_Local_lzo_level () {
...
# イメージファイルの所有者
Func_Global_Ask_owner () {
    local Func_Local_check_user
    Func_Local_check_user () {
        [[ -z "${1+SET}" ]] && return 2
    ...
    if Func_Local_check_user "${Var_Global_iso_owner}"; then
        echo "ユーザーが存在しません。"
        Func_Global_Ask_owner
        return 0
...
Func_Global_Main_create_argument () {
...
Func_Global_Main_run_clean.sh() {
    sudo "${Var_Global_Wizard_Env_script_path}/tools/clean.sh -w ${work_dir}"
}


Func_Global_Main_set_iso_permission() {
...
# 上の質問の関数を実行
Func_Global_Main_ask_questions () {
    Func_Global_Ask_build_arch
    Func_Global_Ask_plymouth
    Func_Global_Ask_kernel
    Func_Global_Ask_comp_type
    ...
    Func_Global_Ask_username
    Func_Global_Ask_password
    # Func_Global_Ask_japanese この関数はAlterISO2以前を想定されたものです。
    Func_Global_Ask_locale
    Func_Global_Ask_channel
    Func_Global_Ask_tarball
