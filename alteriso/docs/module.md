...
`!` で始まる行は除外エントリです。`!foo` と書くと、それまでのレイヤーで追加された
`foo` を一覧から取り除きます。レイヤーは `packages` → `packages.any` →
`packages.<arch>` の順に処理し、各レイヤー内では追加をすべて適用してから除外を
適用します。同じレイヤーに追加と除外の両方があれば、記述元によらず除外が勝ちます。
後のレイヤーで再度追加すれば一覧に戻ります。追加されていないパッケージの除外は
警告を出して無視します。用途は「共通レイヤーで列挙し、特定アーキテクチャだけ
除外する」ケースです（例: `packages.d/system` に載せたまま `packages.i486` に
`!zram-generator`）。
