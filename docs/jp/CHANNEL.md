...
### 通常のパッケージ
AURのパッケージ以外のパッケージ（リポジトリサーバからバイナリを取得するパッケージ）の一覧は`packagea.<arch>`ディレクトリ以下に配置してください。
...
rootユーザーで`makepkg`のビルドは実行できないため、ビルド用のユーザーがスクリプトによって自動的に作成されます。  
この機能はまだ不完全で、今後仕様や内部処理が変更される可能性があります。
