...
_msg_info() {
    ...
    shift $((OPTIND - 1))
    echo ${echo_opts} "$( echo_color -t '36' '[alteriso2.sh]')    $( echo_color -t '32' 'Info') ${*}"
...
_msg_warn() {
    ...
    shift $((OPTIND - 1))
    echo ${echo_opts} "$( echo_color -t '36' '[alteriso2.sh]') $( echo_color -t '33' 'Warning') ${*}" >&2
    if [[ "${bash_debug}" = true ]]; then
        set -xv
    else
        set +xv
...
_msg_debug() {
    ...
    shift $((OPTIND - 1))
    if [[ ${debug} = true ]]; then
        echo ${echo_opts} "$( echo_color -t '36' '[alteriso2.sh]')   $( echo_color -t '35' 'Debug') ${*}"
    fi
    if [[ "${bash_debug}" = true ]]; then
        set -xv
    else
...
_msg_error() {
    ...
    shift $((OPTIND - 1))
    echo ${echo_opts} "$( echo_color -t '36' '[alteriso2.sh]')   $( echo_color -t '31' 'Error') ${1}" >&2
    if [[ -n "${2:-}" ]]; then
        exit ${2}
    fi
    if [[ "${bash_debug}" = true ]]; then
